...
const DRIVE_FOLDER_ID = "1lhhK2BHeq5t7mNLDVkTY5n8eHFe1CdKU"
...
function AccountDataPage() {
  ...
  const fetchSheetData = async () => {
    try {
      ...
      const response = await fetch(`https://docs.google.com/spreadsheets/d/1A0Px7LPEr-kYLiIzAuojBb24fw79STH8lx5kae5Rp5U/gviz/tq?tqx=out:json&sheet=SLAG CRUSHER`);
                            ...
                            >
                              {header.id === 'col14' ? (
                                ...
                                <div className="flex gap-2 flex-wrap">
                                  {history[header.id].split(',').map(url => url.trim()).filter(Boolean).map((url, index) => (
                                    <a
                                      key={index}
                                      href={url}
                                      target="_blank"
